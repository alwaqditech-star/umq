...
export function EditorLayout({
  ...
  const titleMap: Record<string, string> = {
    editor: locale === "ar" ? "لوحة المحرر" : "Editor dashboard",
    blog: dict.admin.blog,
    projects: dict.admin.projects,
    media: locale === "ar" ? "الوسائط" : "Media",
    account: locale === "ar" ? "الحساب" : "Account",
  };
  const pageTitle = titleMap[segment ?? "editor"] ?? titleMap.editor;

  const navLabels: Record<string, string> = {
    dashboard: locale === "ar" ? "لوحة المحرر" : "Editor",
    blog: dict.admin.blog,
    projects: dict.admin.projects,
    media: locale === "ar" ? "الوسائط" : "Media",
    account: locale === "ar" ? "الحساب" : "Account",
  ...
  return (
    <div className="flex min-h-screen bg-background">
      ...
      <Sidebar
        locale={locale}
        homeHref="/editor"
        navItems={filterNavByPermissions(editorNavItems, hasPermission)}
        labels={navLabels}
        panelLabel="Editor navigation"
      />
